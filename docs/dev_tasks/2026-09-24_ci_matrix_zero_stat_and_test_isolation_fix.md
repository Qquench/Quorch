...
[MODIFY] docs/cross_platform_ci_anomalies.md
```
...
目标：以 try/except FileNotFoundError 幂等删除替代存在性检查，支持 Windows PermissionError 优雅降级；用定向白名单过滤与子进程隔离重构测试，并在 docs/cross_platform_ci_anomalies.md 中归档沉淀案例 6。
```
...
def gc_by_filename_order(
    ...
    *,
    ...
    cf. cross_platform_ci_anomalies.md Case 4) is logged and skipped.
    """
...
5. 文档沉淀：在 docs/cross_platform_ci_anomalies.md 中追加案例 6（全局 monkeypatch stdlib C 层函数导致 pytest session 级崩溃），并补充安全编码准则第 5 条。

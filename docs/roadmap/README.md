...
| :--- | :--- | :--- | :---: |
| **v1.07** | ✔️ **[v1.07_architecture_doc_integrity_and_neutrality_decoupling.md](./archive/v1.07_architecture_doc_integrity_and_neutrality_decoupling.md)** | **Architecture Integrity, Neutrality Decoupling & Cross-Paradigm Handoff**:<br>• AST dual-signature anchor integrity guard (`test_architecture_doc_anchor_integrity.py`);<br>• Secret leakage & inline credential static rejection (`_looks_like_plaintext_secret`, `_reject_inline_credentials`);<br>• Neutral configuration templates (`.sample.yaml`, `quench_stack.local.yaml` git-ignore);<br>• Structured Handoff Card protocol (`render_handoff_card`, `dev_tasks_export_handoff_card`);<br>• Transparent reviewer identity & soft self-verification warnings (`reviewer_identity`, `self_verification_warning`);<br>• 467/467 automated unit test suite with 100% green coverage across Linux and Windows. | 2026-09-24 |

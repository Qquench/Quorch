# 常见问题排查与技术问答 (Frequently Asked Questions - FAQ)
...
- **原因**：Antigravity IDE 启动 MCP Server 时使用的是全局系统 Python，而非配置了 `fastmcp>=2.0` 的虚拟环境。
- **排查与解决**：
  1. 在 `quorch` 根目录执行安装引导脚手架：
     ```bash
     python scripts/install.py
     ```
     脚手架会自动探测当前激活的虚拟环境或根目录 `.venv`，并将准确的绝对路径填入 `plugins/quench-dev-tasks/mcp_config.json` 与 `hooks.json`。
  2. 验证虚拟环境中依赖完整：
     ```bash

...
- **审查角色纯粹化解耦**：将所有规范、规则、技能、提示卡与 MCP 工具中的审查角色统一抽象为“架构审查模型 (Reviewer)”，审查模型由用户根据自身预算与订阅环境自主选定（如 Claude 3.7 / Opus、GPT-4.5 / o3、Gemini Pro、DeepSeek R1 等），不再与任何具体厂商或单一模型死板绑定；

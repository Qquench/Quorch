...
## 4. Opus 审查交接与多会话复工纪律 (Opus Handoff & Resume Discipline)

为彻底杜绝长会话 Token 污染、保护用户的 Opus 额度并保证架构质量，必须遵循以下交接与复工协议：

> **角色解耦定义**：在 Quench 治理体系中，“Opus”是平台最高架构审查与推理能力模型的**通用角色代号**（特指用户在当前平台上可切换到的最尖端旗舰模型，如 Antigravity 当前环境下的 Opus 4.6 等）。随着平台服务演进，模型系列与版本号可能动态升级，但交互提示与交接卡一律保持精炼的 **Opus** 代称，杜绝版本号硬编码带来过期失效。

### ① 停机与上报触发条件 (When to Halt)
当遇到以下任一情况时，Flash **必须立刻停止后续开发与代码编写工具调用**：
1. 任务单处于 `⬜ 待确认` 或已被标记为 `🔄 需返工`，需要 Opus 架构审查与开发指南终审；
2. **已确认任务的返工挂起**：某些任务虽此前状态为 `✅ 已确认`，但若属于 Flash 生成初稿或用户指定需 Opus 重新修订，必须调用 `dev_tasks_confirm(action="rework")` 降级为 `🔄 需返工`，阻断直接领单施工；
...
### ② 标准上报格式 (Handoff Card)
Flash 停机后，必须在回复中输出格式化交接卡，指导用户进行低成本新会话交接。
...
> [!IMPORTANT]
> ### ⏸️ Quench 任务交接：等待 Opus 架构审查与指南修订
> - **待审查任务单**：`docs/dev_tasks/<文件名>.md` (任务 ID: <待审查ID>)
> - **交接原因**：<清晰说明为何需要高级架构审查 / 返工重修>
> 
> **👉 请执行以下低 Token 消耗的交接操作：**
> 1. 点击 `+` 打开一个【全新会话】（纯净上下文，仅消耗极少 Token）；
> 2. 模型选择 **Opus**（或 `@opus_reviewer`）；
> 3. 发送提示词：`@opus_reviewer 请审查并修订 docs/dev_tasks/<文件名>.md`；
> 4. Opus 完成审查修订并将任务置为 `✅ 已确认`；
> 5. **切回本会话**，输入“`Opus已修订完毕`”或“`继续`”。

### ③ 复工确认机制 (Resume Verification)
当用户切回原会话并给出确认后，Flash 的行为约束如下：
1. **禁止盲目直接开工**：必须首先调用 `dev_tasks_status` 重新拉取磁盘上的最新任务单；
2. **校验任务有效性**：核实目标任务已被 Opus 标记为 `✅ 已确认`，且六大字段清晰规范；
...
### ④ 用户豁免 Opus 时的直通模式与微留痕纪律 (Bypass & Micro-Patch Discipline)
- **风险提示先行**：当用户明确指示“跳过 Opus 审核 / 直接执行”时，Agent 严禁静默开工，必须首先输出【直通模式风险通知卡】（`> [!WARNING]`），向用户声明双模型审查已绕过，进入用户主导驾驶状态。

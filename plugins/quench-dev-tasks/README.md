...
2. **Deterministic State Machine**:
   - Strict unidirectional state progression: `⬜ PROPOSED` ➔ `✅ CONFIRMED` ➔ `🔨 IN_PROGRESS` ➔ `✔️ COMPLETED` (with `🔄 REWORK` and `⏭️ SKIPPED` branches).
...
│   ├── server.py               # 8 core governance tools
│   ├── state_machine.py        # State machine engine + FileLock
│   ├── schema_validator.py     # Six-core-field validator
│   ├── project_config.py       # Project manifest parser
│   ├── changelog_writer.py     # Automated CHANGELOG sync
│   ├── cli.py                  # Unified terminal CLI
│   ├── adapters/               # Multi-client adapter layer
│   ├── hooks/                  # PreToolUse guard & context injector
│   └── tests/                  # Automated test suite (105/105 passed)
...
| :--- | :--- |
| `dev_tasks_status` | Returns task status overview, active session bypasses, and recent audit logs |
| `dev_tasks_propose` | Submits a new task proposal with six-core-field validation |
| `dev_tasks_confirm` | Confirms task documents (`confirm`, `rework`, `skip`) |
...
| `dev_tasks_escalate` | Escalates blocked tasks to a frontier reasoning reviewer subagent |

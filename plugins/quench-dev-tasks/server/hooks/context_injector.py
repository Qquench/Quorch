    ...
    try:
        ...
        if pending_count > 0:
            reminder_msg = (
                ...
                f"1. 默认动作：起草完成或面对未审任务时，必须主动向用户输出规范的【任务交接卡】（⚠️ 排版硬性要求：必须直接使用 GitHub 原生引用块 `> [!NOTE]` 渲染，绝对严禁使用 ASCII 边框画图 `┌──┐`，绝对严禁套在 ```markdown 代码块中做成黑板报/展示板）；\n"
                f"2. 严禁自作主张询问用户是否由自己直接开工；\n"
                f"3. 唯一例外：仅当用户在提示词中显式明确要求“本单无需审查 / 跳过审查 / 直接执行”时，方可豁免交接。"
            )
            print(
                json.dumps(
                    {
